...
/**
 * Cloud-first service for PaymentMethod.
 * Implements dual-writing: Postgres if online, SQLite + Queue if offline.
 */
export class PaymentMethodCloudService extends CloudFirstBaseService<
  PaymentMethod,
  CreatePaymentMethodDto,
  UpdatePaymentMethodDto
> {
  ...
  private mapRow(row: any): PaymentMethod {
    return {
      id: row.id,
      name: row.name,
  ...
  private saveToLocal(entity: PaymentMethod): void {
    const existing = this.findByIdLocalSync(entity.id)
    if (existing) {
      this.localDb.run(
        `UPDATE payment_method SET name = ?, is_active = ?, created_at = ?, updated_at = ?, deleted_at = ?, synced_at = ? WHERE id = ?`,
        [
          entity.name,
          entity.isActive ? 1 : 0,
      ...
      this.localDb.run(
        `INSERT INTO payment_method (id, name, is_active, created_at, updated_at, deleted_at, synced_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          entity.id,
          entity.name,
          entity.isActive ? 1 : 0,
  ...
  private findAllLocal(): PaymentMethod[] {
    const stmt = this.localDb.prepare('SELECT * FROM payment_method WHERE deleted_at IS NULL ORDER BY name ASC')
  ...
  private findActiveLocal(): PaymentMethod[] {
    const stmt = this.localDb.prepare('SELECT * FROM payment_method WHERE deleted_at IS NULL AND is_active = 1 ORDER BY name ASC')
    const results: PaymentMethod[] = []
    while (stmt.step()) {
      results.push(this.mapLocalRow(stmt.getAsObject()))
    }
  ...
  private findByIdLocalSync(id: string): PaymentMethod | undefined {
    const stmt = this.localDb.prepare('SELECT * FROM payment_method WHERE id = ?')
    stmt.bind([id])
    if (stmt.step()) {
  ...
  private async findByIdLocal(id: string): Promise<PaymentMethod | undefined> {
    return this.findByIdLocalSync(id)
  }

  private async createLocal(id: string, data: CreatePaymentMethodDto, now: Date): Promise<PaymentMethod> {
    this.localDb.run(
    ...
    saveDb(this.localDb)
    return this.findByIdLocal(id) as Promise<PaymentMethod>
  }

  private async updateLocal(id: string, data: UpdatePaymentMethodDto, now: Date): Promise<PaymentMethod> {
  ...
  private async softDeleteLocal(id: string, now: Date): Promise<PaymentMethod> {
    this.localDb.run('UPDATE payment_method SET deleted_at = ?, updated_at = ? WHERE id = ?', [
      now.getTime(),
      now.getTime(),
      id
    ])
  ...
  private mapLocalRow(row: any): PaymentMethod {
    return {
      id: row.id as string,
