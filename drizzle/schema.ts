...
export const uom = pgTable(
  ...
  (table) => [unique('uom_code_unique').on(table.code)]
)
...
export const productUomCategoryPrice = pgTable(
  ...
  (table) => [
    unique('product_uom_category_price_product_id_uom_id_price_category_id_').on(
      table.productId,
      table.uomId,
...
export const productUom = pgTable('product_uom', {
  ...
  cost: numeric(), // Cost for this UOM (null = auto-calculate from base)
  costOverride: boolean('cost_override').default(false).notNull(), // If true, use cost field; if false, calculate from base
  createdAt: timestamp('created_at', { mode: 'string' }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow().notNull(),
  syncedAt: timestamp('synced_at', { mode: 'string' }),
  deletedAt: timestamp('deleted_at', { mode: 'string' }),
...
export const storeProductUomPrice = pgTable(
  ...
  (table) => [
    unique('store_product_uom_price_product_id_uom_id_price_category_id_sto').on(
      table.productId,
      table.uomId,
      table.priceCategoryId,
      table.storeId
...
export const deliveryOrder = pgTable(
  ...
  (table) => [unique('delivery_order_no_surat_jalan_unique').on(table.noSuratJalan)]
)
...
export const transactions = pgTable(
  ...
  (table) => [unique('transactions_code_unique').on(table.code)]
)
...
export const employee = pgTable(
  ...
  (table) => [unique('employee_code_unique').on(table.employeeCode)]
)
...
export const employeeSalaryComponent = pgTable(
  ...
  (table) => [unique('employee_salary_component_unique').on(table.employeeId, table.componentId)]
)
...
export const salaryComponent = pgTable(
  ...
  (table) => [unique('salary_component_code_unique').on(table.code)]
)
...
export const attendance = pgTable(
  ...
  (table) => [unique('attendance_employee_date_unique').on(table.employeeId, table.date)]
)
