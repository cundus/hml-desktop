...
export class CustomerCategoryCloudService {
  ...
  async findAll(): Promise<CustomerCategory[]> {
    if (this.isOnline()) {
      try {
        ...
        const result = await pool.query('SELECT * FROM customer_category WHERE deleted_at IS NULL ORDER BY name ASC')
  ...
  private findAllLocal(): CustomerCategory[] {
    const stmt = this.localDb.prepare('SELECT * FROM customer_category WHERE deleted_at IS NULL ORDER BY name ASC')
  ...
  async create(data: CreateCustomerCategoryDto): Promise<CustomerCategory> {
    ...
    const cat: CustomerCategory = { id, name: data.name, createdAt: now, updatedAt: now, syncedAt: null, deletedAt: null }

    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('INSERT INTO customer_category (id, name, created_at, updated_at) VALUES ($1, $2, $3, $4)',
    ...
    this.localDb.run('INSERT INTO customer_category (id, name, created_at, updated_at) VALUES (?, ?, ?, ?)',
      [id, data.name, now.getTime(), now.getTime()])
    saveDb(this.localDb)
    await this.queueService.add('INSERT', this.tableName, { id, name: data.name, created_at: now.toISOString(), updated_at: now.toISOString() })
    return cat
  }
  ...
  async update(id: string, data: UpdateCustomerCategoryDto): Promise<CustomerCategory> {
    ...
    const now = new Date()
    const updated: CustomerCategory = { ...existing, name: data.name ?? existing.name, updatedAt: now }

    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE customer_category SET name = $1, updated_at = $2 WHERE id = $3', [updated.name, now, id])
        return updated
      } catch (error) {
    ...
    this.localDb.run('UPDATE customer_category SET name = ?, updated_at = ? WHERE id = ?', [updated.name, now.getTime(), id])
    saveDb(this.localDb)
    await this.queueService.add('UPDATE', this.tableName, { id, name: updated.name, updated_at: now.toISOString() })
    return updated
  }
  ...
  async softDelete(id: string): Promise<CustomerCategory> {
    ...
    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE customer_category SET deleted_at = $1, updated_at = $2 WHERE id = $3', [now, now, id])
        return deleted
      } catch (error) {
    ...
    this.localDb.run('UPDATE customer_category SET deleted_at = ?, updated_at = ? WHERE id = ?', [now.getTime(), now.getTime(), id])
    saveDb(this.localDb)
  ...
  async restore(id: string): Promise<CustomerCategory> {
    ...
    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE customer_category SET deleted_at = NULL, updated_at = $1 WHERE id = $2', [now, id])
      } catch (error) {
        console.error('[CustomerCategoryCloud] restore error:', error)
      }
    }
    this.localDb.run('UPDATE customer_category SET deleted_at = NULL, updated_at = ? WHERE id = ?', [now.getTime(), id])
    saveDb(this.localDb)
