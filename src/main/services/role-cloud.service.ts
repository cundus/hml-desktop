...
export class RoleCloudService {
  ...
  async findAll(): Promise<Role[]> {
    if (this.isOnline()) {
      try {
        ...
        const result = await pool.query('SELECT * FROM role WHERE deleted_at IS NULL ORDER BY name ASC')
  ...
  private findAllLocal(): Role[] {
    const stmt = this.localDb.prepare('SELECT * FROM role WHERE deleted_at IS NULL ORDER BY name ASC')
    const results: Role[] = []
  ...
  private findByIdLocal(id: string): Role | undefined {
    ...
    stmt.bind([id])
    if (stmt.step()) { const r = this.mapLocalRow(stmt.getAsObject()); stmt.free(); return r }
    stmt.free()
    return undefined
  }
  ...
  async create(data: CreateRoleDto): Promise<Role> {
    const id = randomUUID()
    const now = new Date()
    const role: Role = { id, name: data.name, description: data.description ?? null, createdAt: now, updatedAt: now, syncedAt: null, deletedAt: null, deviceId: null }

    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('INSERT INTO role (id, name, description, created_at, updated_at) VALUES ($1, $2, $3, $4, $5)',
    ...
    this.localDb.run('INSERT INTO role (id, name, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
      [id, data.name, data.description ?? null, now.getTime(), now.getTime()])
    saveDb(this.localDb)
    await this.queueService.add('INSERT', this.tableName, { id, name: data.name, description: data.description ?? null, created_at: now.toISOString(), updated_at: now.toISOString() })
    return role
  }
  ...
  async update(id: string, data: UpdateRoleDto): Promise<Role> {
    ...
    const now = new Date()
    const updated: Role = { ...existing, name: data.name ?? existing.name, description: data.description ?? existing.description, updatedAt: now }

    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE role SET name = $1, description = $2, updated_at = $3 WHERE id = $4',
    ...
    this.localDb.run('UPDATE role SET name = ?, description = ?, updated_at = ? WHERE id = ?',
      [updated.name, updated.description, now.getTime(), id])
    saveDb(this.localDb)
    await this.queueService.add('UPDATE', this.tableName, { id, name: updated.name, description: updated.description, updated_at: now.toISOString() })
    return updated
  }
  ...
  async softDelete(id: string): Promise<Role> {
    ...
    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE role SET deleted_at = $1, updated_at = $2 WHERE id = $3', [now, now, id])
        return deleted
      } catch (error) {
    ...
    this.localDb.run('UPDATE role SET deleted_at = ?, updated_at = ? WHERE id = ?', [now.getTime(), now.getTime(), id])
    saveDb(this.localDb)
  ...
  async restore(id: string): Promise<Role> {
    ...
    if (this.isOnline()) {
      try {
        const pool = getCloudDb().getPool()
        await pool.query('UPDATE role SET deleted_at = NULL, updated_at = $1 WHERE id = $2', [now, id])
      } catch (error) {
        console.error('[RoleCloud] restore error:', error)
      }
    }
    this.localDb.run('UPDATE role SET deleted_at = NULL, updated_at = ? WHERE id = ?', [now.getTime(), id])
    saveDb(this.localDb)
